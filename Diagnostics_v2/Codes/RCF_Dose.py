...
def interpolate_calibration_hdv2(scanner = "Epson_12000XL"):
    '''
    Interpolates calibration data of HDV2 layers

    Args:
        scanner: name of camera used to scan images

    Returns:
        function used to match 16-bit RGB values to dose
    '''
    calibration_path = ""
    ...
    def cs_hdv2(rgb: np.ndarray) -> float:
        if np.mean([red_cs(rgb[0]), green_cs(rgb[1]), blue_cs(rgb[2])]) > 4:
            return 0
        return np.power(10, np.mean([red_cs(rgb[0]), green_cs(rgb[1]), blue_cs(rgb[2])]))

    return cs_hdv2

def interpolate_calibration_ebt3(scanner = "Epson_12000XL"):
    '''
    Interpolates calibration data of EBT3 layers
...
def convert_to_dose(project: str, shot: str) -> list[np.ndarray]:
    ...
    func_hdv2, func_ebt3 = interpolate_calibration_hdv2(), interpolate_calibration_ebt3()
    converted_imgs = []
